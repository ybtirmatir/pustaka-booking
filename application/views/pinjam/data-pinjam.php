								...
								<td>
									...
									<td>
										<?php $tgl1 = new DateTime($p['tgl_kembali']);
										     $tgl2 = new DateTime();
											 $selisih = $tgl2->diff($tgl1)->format("%a"); echo $selisih;?> Hari
 									</td>
									<td>
										<?= $p['denda']; ?>
									</td>
									<?php if ($p['status'] == "Pinjam") {$status = "warning";
									} 
									else { $status = "secondary";
									}?>
									<td> <i class="btn btn-outline-<?= $status; ?> btn-sm">
											<?= $p['status']; ?>
										</i>
									</td>
									...
									<td>
										<?= $total_denda; ?>
										<input type="hidden" name="totaldenda" id="totaldenda" value="
											<?= $total_denda; ?>">
										</td>
										<td nowrap>
